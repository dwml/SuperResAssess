...
def main(experiment_configuration_path, dataset_path, lightning_module_config_path):
    ...
    test_result = assessment_method.internal_test(
        assess_result.log_versions, assess_result.best_model_path
    )
    write_configuration_as_yaml(
        test_result, experiment_log_dir.joinpath("internal_test_result.yaml")
    )
